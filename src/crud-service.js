const { ValidationError, NotFoundError } = require('mini-mvcs-errors');
const sequelizeErrorBuilder = require('./sequelize-error-builder');
...
const validateWrapper = (model) => {
  return (instance) => {
    ...
    .catch((sequelizeValidationError) => {
      return new ValidationError(model.name, sequelizeErrorBuilder(sequelizeValidationError));
    })
...
const read = (model) => {
  return (id) => {
    ...
    .then((instance) => {
      if (!instance) throw new NotFoundError(model.name, typeof id === 'object' ? id : { id });
      return instance;
    });
  };
};
  ...
  return (params) => {
    return model.build({
      ...
      ...(
        params.auditUsuario
          ? {
            usuarioCreacion: params.auditUsuario.id,
            usuarioModificacion: params.auditUsuario.id,
          }
          : {}
      ),
    });
...
// shortcut method for updating certain attributes of a sequelize instance
const setAndUpdate = (instance, params) => {
  instance.set(params);
  return instance.save();
};
...
crudService.setAndUpdate = setAndUpdate;
